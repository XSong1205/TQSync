# TQSync - Telegram & QQ 双向消息同步机器人

TQSync 是一个基于 Python 的异步机器人，用于实现 Telegram 群组与 QQ 群组（通过 Napcat OneBot 协议）之间的双向消息同步。

## 功能特性

- **双向文本同步**：实时转发 TG 和 QQ 群组的文本消息。
- **用户绑定系统**：支持通过 `/bind` 指令将 TG 用户与 QQ 用户关联，转发时显示对方平台的昵称。
...
- **QQ**: `aiohttp` (对接 Napcat OneBot v11 HTTP API)
